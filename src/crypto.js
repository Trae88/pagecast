...
// escaping, and references the three constants prepended by renderAuthMiddleware
// (PROTECTED, COOKIE_SECRET, GATE_HTML).
...
export function renderAuthMiddleware(manifest, { cookieSecret = "", badge = true } = {}) {
  ...
  for (const entry of manifest || []) {
    if (!entry || !entry.slug || !isValidPasswordHash(entry)) {
      continue;
    }
    protectedMap[entry.slug] = {
      salt: entry.salt,
      hash: entry.hash,
  ...
  const header = [
    ...
    "// Edge password gate for protected /p/<slug>/ pages (HTTP Basic Auth +",
    "// signed session cookie). The password hash is never served as an asset.",
    `const PROTECTED = ${JSON.stringify(protectedMap)};`,
